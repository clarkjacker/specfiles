...
#WARNING: If you change --prefix to /usr, comment out the part that symlinks 'python' to 'python2.7'
...
%changelog
* Fri Feb 15 2013 Ben Clark <bclark@wayfair.com> [2.7.3-milford]
- Upgrade to 2.7.3, changed 'Provides' section as needed by CentOS 6.
- Hackishly fixed the #! in python-config.
- Changed prefix to /usr/local and added a symlink so env scripts just work.
...
#  hack to fix the #! line in python%{libvers}-config
cd $RPM_BUILD_ROOT%{__prefix}/bin
sed -i -e 's|#!.*python.*|#!%{__prefix}/bin/python%{libvers}|' python%{libvers}-config

#  add sym link
ln -s python%{libvers} python
